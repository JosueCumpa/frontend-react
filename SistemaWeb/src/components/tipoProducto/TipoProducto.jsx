...
import AgregarTipoProducto from './AgregarTipoProducto.jsx';
import EditarTipoProducto from './EditarTipoProducto.jsx';
import EliminarTipoProducto from './EliminarTipoProducto.jsx';
    ...
    const list = useAsyncList({
        async load({ signal }) {
            try {
                ...
                const res = await axios.get(`${API_BASE_URL}/TipoProducto/`, {
                    headers: {
                ...
                if (error.response && error.response.status === 401) {
                    try {
                        ...
                        const refreshedRes = await axios.get(`${API_BASE_URL}/TipoProducto/`, {
                            headers: {
            ...
            <Breadcrumb
                items={[
                    ...
                    },
                    {
                        title: (
                            <>
                                <span>Gestion de Tipo de Producto</span>
                            </>
                        ),
                    },
                            ...
                            .map((item) => (
                                <TableRow key={item?.id}>
                                    ...
                                    <TableCell className="flex relative gap-3">
                                        <EditarTipoProducto tipoProducto={item} updateTipoProducto={updateTipoProducto}></EditarTipoProducto>
                                        <EliminarTipoProducto tipoProducto={item} updateTipoProducto={updateTipoProducto}></EliminarTipoProducto>
                                    </TableCell>
                                </TableRow>
                            ))}
                    </TableBody>
